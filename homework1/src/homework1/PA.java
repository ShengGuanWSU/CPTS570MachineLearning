...
public class PA {
    ...
    private static final double[][] w= new double[10][129];
	...
	public static void weightCal(String trainFile, int sampleFileI) throws IOException{
        ...
        double ytt; //yt hat, predict using the current weights
        	for(int j = 1; j <= trainData.size; j++){
        		...
        		ytt = dotProduct(w[sampleFileI], trainData.featureValue[j],128);
        		if((ytt * trainData.y[j]) < 1.0){
        			trainingMistakes[sampleFileI] ++;
        			tao = Math.max(0.0,(1-trainData.y[j]*dotProduct(w[sampleFileI],trainData.featureValue[j],128))/dotProduct(trainData.featureValue[j],trainData.featureValue[j],128));
        			for(int k = 1; k <= 128; k++){
        				w[sampleFileI][k] = w[sampleFileI][k] + tao *trainData.y[j] * trainData.featureValue[j][k];
    ...
    */
	...
	public static void main(String args[]) throws IOException{
		...
		String csvFile = testFileNameBase+"PA_Test_Accuracy.csv";
