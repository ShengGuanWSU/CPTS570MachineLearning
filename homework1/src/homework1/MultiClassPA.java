...
public class MultiClassPA {
	...
	public static int sign(double[] w, int[] xt, int featureSize){
		double res = 0.0;
	...
	public static double dotProduct(double[] w, int[] xt, int size){
		double res = 0.0;
	...
	public static int dotProduct(int[] w, int[] xt, int size){
		int res = 0;
		for(int i = 1; i<= size; i++)
			res += w[i] * xt[i];
		return res;
	...
	public static int argMax(double[] multiWeight, int data[]) {
        ...
        for (int k = 0; k <26; k++) {
            ...
            double currWX = dotProduct(multiWeight,currMultiX,128*26);
	  ...
	  public static void UpdateWeightMultiClassPA(double[] w, int ytt, int yGround, int[] data) {
	        ...
	        double tau=0.0;
	        double arg1 = dotProduct(w,FXY,128*26);
	        double arg2 = dotProduct(w,FXYtt,128*26);
	        int arg3 = 0;
	        double temp;
	        ...
	        //arg3=dotProduct(temp, temp, 128*26);
	        tau = (1 - (arg1 - arg2)) / arg3;

	        for (int i = 1; i <=128*26; i++) {
	            w[i] = w[i]+ tau * (FXY[i] - FXYtt[i]);
	        }

	    }
	 
	  public static double loss(double[] w,int[] FXY,int[]FXYtt){
		
		if (dotProduct(w, FXYtt,128*26)-dotProduct(w,FXY,128*26)+1<0)
			return 0;
		else 
			return (double) (dotProduct(w, FXYtt,128*26)-dotProduct(w,FXY,128*26)+1);
	}
	
	public static double loss(int ytt, int yt){
	...
	public static void weightCal(String trainFile, int sampleFileI) throws IOException{
        ...
        int ytt; //yt hat, predict using the current weights
        	for(int j = 1; j <= trainData.size; j++){
    	        ...
    	        for(int i = 1; i <= 128; i++) {
    	            FXY[(128 * trainData.y[j]) + i] =  trainData.featureValue[j][i];
    	            FXYtt[(128 * ytt) + i] = trainData.featureValue[j][i];
    	        }
    	        
    	        //if (ytt != trainData.y[j]){
    	        //if((dotProduct(w[sampleFileI], FXY,128*26)-dotProduct(w[sampleFileI], FXYtt,128*26))<1){
        		if((dotProduct(w[sampleFileI], FXY,128*26)-dotProduct(w[sampleFileI], FXYtt,128*26))<loss(trainData.y[j],ytt)){
        		//if((dotProduct(w[sampleFileI], FXY,128*26)-dotProduct(w[sampleFileI], FXYtt,128*26))<loss(w[sampleFileI],FXY,FXYtt)){
        			//for(int k = 1; k <= 128; k++){
        				//w[sampleFileI][k] = w[sampleFileI][k] + trainData.y[j] * trainData.featureValue[j][k];	
        			UpdateWeightMultiClassPA(w[sampleFileI], ytt, trainData.y[j], trainData.featureValue[j]);
	...
	*/
	...
	public static Vector<String> createVectorFromString(String sContent,
		...
		public static void main(String args[]) throws IOException{
			...
			//String csvFile = testFileNameBase+ "PA_Iteration_AverageMistakes.csv";
			//String csvFile = testFileNameBase+"PA_Iteration_Accuracy.csv";
			String csvFile = testFileNameBase+"PA_Test_MultiClass_Accuracy.csv";
			FileWriter writer = new FileWriter(csvFile);
			//CSVUtils.writeLine(writer, Arrays.asList("Iterations","AverageMistakes"));
			CSVUtils.writeLine(writer, Arrays.asList("Iterations","Accuracies"));
